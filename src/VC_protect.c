...
uint8_t CUV_Comp(uint16_t threshold)
{
    uint16_t min_Vcell = CellVoltage[0];
    for(int i=1;i<16;i++)
    {
...
void COV_protect()
{
    switch (COV_State)
    {
        ...
        case ALERT:
            ...
            if(COV_counter == COV_DLY-1 && COV_Comp(COV_TH))
            {
                COV_counter = 0;
                COV_State = TRIP;
				printf("Error: COV\n");
...
void CUV_protect()
{
    switch (CUV_State)
    {
        ...
        case ALERT:
            ...
            if(CUV_counter == CUV_DLY-1 && CUV_Comp(CUV_TH))
            {
                CUV_counter = 0;
                CUV_State = TRIP;
				printf("Error: CUV\n");
...
void COVL_protect()
{
    switch (COVL_State)
    {
        ...
        case LATCH_ALERT:
            ...
            else if(COVL_DEC_counter >= COVL_DEC_DLY)    
                COVL_counter --;
            break;
        case LATCH_TRIP:
            clearFlags(COVL_ALERT,SafetyAlertC);		//Safety Alert C[COVL] = 0;
